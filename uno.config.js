...
export default defineConfig({
  ...
  shortcuts: {
    'navbar': 'h-[70px] bg-white/95 dark:bg-surface-900/95 backdrop-blur-md border-b border-surface-200 dark:border-surface-800 fixed top-0 w-full z-50 flex items-center px-4 transition-all duration-300',
    ...
    'nav-item': 'no-underline text-surface-600 dark:text-surface-300 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 hover:text-sky-600 transition',
    ...
    'icon-btn': 'bg-transparent border-none w-10 h-10 rounded-full cursor-pointer flex items-center justify-center text-surface-600 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-surface-800 hover:text-sky-600 transition relative active:scale-95',
    ...
    'btn-google-login': 'bg-white dark:bg-surface-800 text-surface-900 dark:text-white border border-surface-200 dark:border-surface-700 px-4 py-2 rounded-xl font-bold text-xs sm:text-sm cursor-pointer flex items-center gap-2 transition hover:bg-surface-50 shadow-xs',
    ...
    'product-card': 'bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-800 overflow-hidden flex flex-col transition hover:-translate-y-1.5 shadow-sm hover:shadow-md no-underline relative',
    'product-card-img': 'w-full h-48 object-cover border-b border-surface-200 dark:border-surface-800 transition duration-500',
    ...
    'product-card-btn': 'w-full p-2.5 bg-surface-50 dark:bg-surface-800 text-sky-600 dark:text-sky-400 text-center font-bold text-xs border-t border-surface-200 dark:border-surface-800 transition',
    'filter-chip': 'px-4 py-1.5 rounded-full bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 font-semibold text-xs cursor-pointer transition border border-transparent hover:bg-surface-200 no-underline inline-block',
    'filter-chip-active': 'px-4 py-1.5 rounded-full bg-[#0284c7] text-white font-bold text-xs cursor-pointer transition shadow-sm no-underline inline-block',
